"""Product-owned seed content for diversolab_app.
...
splent_feature_about, and its images ship in seed_media/ next to this file.

Content migrated from diversolab.us.es: the "About Us" page plus the homepage
intro blurb, and the "Our values" page (English version), with the Elementor
markup cleaned down to semantic HTML.
"""
...
def _seed_image(filename, title=""):
    """Import a bundled photo through the media feature and return its URL."""
    if MediaService is None:
        return ""
    path = os.path.join(os.path.dirname(__file__), "seed_media", filename)
    # Same source keys the feature seeder used, so a database seeded before
    # this content moved into the product does not reimport the photos.
...
# Photos shown between the two subsections, in order.
WHO_WE_ARE_IMAGES = [
